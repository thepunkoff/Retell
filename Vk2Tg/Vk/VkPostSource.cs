...
using Poll = Vk2Tg.Core.Models.Poll;
using Post = Vk2Tg.Core.Models.Post;
...
public class VkPostSource : IPostSource
{
    ...
    /// <inheritdoc />
    public async IAsyncEnumerable<Post> GetPosts([EnumeratorCancellation] CancellationToken token)
    {
    ...
    private bool TryMapToDomainPost(WallPost? vkPost, [NotNullWhen(true)] out Post? post)
    {
        post = null;
        ...
        foreach (var attachment in vkPost.Attachments)
        {
            switch (attachment.Instance)
            {
                ...
                case VkNet.Model.Attachments.Poll vkPoll:
                    poll = new Poll(vkPoll.Question, vkPoll.Answers.Select(x => x.Text).ToArray(), vkPoll.Multiple ?? false);
                    break;
                case Link vkLink:
        ...
        post = new Post(text, media.Count > 0 ? media.ToArray() : null, poll, links.Count > 0 ? links.ToArray() : null);
        return true;
    }
